...
class ChatRoomFavoritePersistenceAdapter(
    ...
    private fun saveInternal(chatRoomFavorite: ChatRoomFavorite): ChatRoomFavorite {
        val favoriteId = chatRoomFavorite.id
        val savedEntity = if (favoriteId != null) {
            // 업데이트
            val existingEntity = repository.findById(favoriteId.value).orElseThrow {
                IllegalStateException("즐겨찾기를 찾을 수 없습니다. id=$favoriteId")
            }
